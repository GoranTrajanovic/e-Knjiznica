...
if (isset($_SESSION['current_user_id'])) { session_destroy(); }
...
if (isset($_POST['set_ime_ucenika'])) {
  ...
  if (empty($_POST['set_ozn_ucenika']) || empty($_POST['set_ime_ucenika']) || empty($_POST['set_prezime_ucenika']) || empty($_POST['set_email_ucenika']) || empty($_POST['set_uid_ucenika']) || empty($_POST['set_pwd_ucenika'])) {
		header('Location: .?set_ucenik=empty');
	} else {
    try {
...
try {
	// $result = $pdo->query('SELECT * FROM posudjene_knjige WHERE br_preostalih_dana < 0');
	$sql = "SELECT br_preostalih_dana FROM posudjene_knjige WHERE br_preostalih_dana < 0";
